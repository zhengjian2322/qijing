...
if (isset($Action) && (($Action == "Edit") or ($Action == "Add"))) {
    ...
    if ($Id >= 0) /* -1 for new users, or >=0 for existing ones */ {
        ...
        // safe.
        if (!$data) {
            trigger_error("没有此ID的用户", E_USER_NOTICE);
    ...
    if ($Action == "Edit") {
        $n_users = db()->query1("SELECT COUNT(*) FROM $tbl_users WHERE 1");
        $editing_last_admin = $n_users <= 1 ? true : false;
    }
    ...
    foreach ($fields as $field) {
        ...
        switch ($key) {
            ...
            case 'name':
                ...
                $fieldset->addElement(get_field_name($params));
                break;
            default:
                $params['field'] = "users.$key";
                $fieldset->addElement(get_field_custom($field, $params));
                break;
...
if (!$ajax) {
    ...
    echo "<h1>" . get_vocab("user_list") . "</h1>\n";
...
if ($initial_user_creation != 1)   // don't print the user table if there are no users
{
    ...
    $res = db()->query("SELECT * FROM $tbl_users ORDER BY `id` DESC, name");
    ...
    if (!$ajax) {
        ...
        echo "<table class=\"admin_table display\" id=\"users_table\" style=\"font-size: 20px\">\n";
        ...
        // First column which is the name
        echo '<th><span class="normal" data-type="title-string" style="font-size: 20px">' . "姓名" . "</th>\n";
        ...
        foreach ($fields as $field) {
            ...
            if (!in_array($fieldname, $ignore_columns)) {
                ...
                switch ($fieldname) {
                    case 'timestamp':
                        $heading = '<span class="normal" data-type="title-numeric" style="font-size: 20px">' . "上次更改的时间" . '</span>';
                        break;
                    case 'last_login':
                        $heading = '<span class="normal" data-type="title-numeric" style="font-size: 20px">' ."上次登录的时间" . '</span>';
                        break;
                        break;
                    default:
                        break;
